...
/*
 	...
 	인스턴스 변수: 인스턴스가 생성될 떄마다 각 인스턴스별로 초기화
 	...
 	관리 영역 밖에 존재하기에 static 영억에 있는 멤버들은 프로그램의
 	종료시까지 메모리가 할당된 채로 존재하게 된다.
 	남발하면 시스템 성능에 악영향은 미침
 */
...
class Product {
	...
	static String productName;	//클래스 변수 or 정적변수(클래스 초기화)
	int num = 1;	//인스턴스 변수(명시적 초기화)
	
	// 클래스 초기화 블럭 (static 영역에 존재)
	static {
		productName = "SCIT47";
		System.out.println("static: " + productName);
		System.out.println("클래스 초기화 블럭 실행"); 	//static이라 한 번만 실행
	}
	
	// 인스턴스 초기화 블럭
	{
		count++;	//static 영역에 있는 변수라 처음에만 초기화 후 여기서 증가됨
		serialNo = count;
