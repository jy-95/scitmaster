...
/*
 	...
 	클래스, 메서드, 변수 등에 대한 접근 범위를 지정하여 외부로부터의 접근을 제어
 	 ...
 	 - protected:
 	 클래스 = 클레스에는 사용할 수 없음
 	 메서드/변수 = 같은 클래스, 같은 패키지, 상속관계(다른 패키지의 하위 클래스에서 접근 가능)
 	 			의 클래스에서만 접근 가능
 	 - default: 
...
//		System.out.println("pw: " + user.pw);	//pw는 private이므로 이 클래스에서는 볼 수 없다.
		System.out.println("pw: " + user.pwCheck());
		
		System.out.println("=== 변경 후 ===");
		user.id = "이순신";
//		user.pw = "1234";	//pw는 직접적인 접근 불가. err
		System.out.println("id: " + user.id);
		System.out.println("pw: " + user.pwCheck());
	}
